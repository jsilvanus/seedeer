...
> **Status: Phase 1 complete.** Image embeddings (`JointEmbedder`,
> `VisualEmbedder`) are real and working, across all four execution modes
> (`process`/`thread`/`socket`/`grpc`). Captioning, VQA, and
> detection/tracking are still stubs that throw `NotImplementedError`
